...
for index, item in enumerate(items):
    ...
    history = model.fit(x_train, y_train, validation_data=(x_test, y_test), epochs=epochs, batch_size=batch_sizes[index], verbose=VERBOSE, callbacks=[earlyStop, LearningRateScheduler(scheduler)], validation_batch_size=1_000)
    histories.append(history)

    model.save(f"models/combinedgap-{item}.h5")
...
print_to_file(dicts, mccs, items, epochs, batch_sizes, learning_rate, patiences, last_epochs, model, 'combined_GAP')
make_graphs(histories, items, 'combined_GAP')