...
def scheduler(epoch, lr):
    ...
    max_iter=20
    delta=10
    clr = lrmin + ((lrmax-lrmin)*(1.-fabs((epoch/step_size)-(2*floor(epoch/(2*step_size)))-1.)))
    clr_decay = clr/(1.+((delta-1.)*(epoch/max_iter)))
    return clr_decay

def clr(x_train, y_train, x_test, y_test, ep, bs, verb=0):
    ...
    model.fit(x_train, y_train, epochs=ep, batch_size=bs, verbose=verb, callbacks=[LearningRateScheduler(scheduler)])
...
if __name__ == "__main__":
    ...
    epochs = 20
    batch_size = 64
    results = clr(x_train, y_train, x_test, y_test, epochs, batch_size, verb=1)
    print(results)